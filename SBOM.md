# Software Bill of Materials (SBOM)

Software Bill of Materials (SBOM) listing the software components and dependencies used by AAPP-MART, including their versions and license information.

| Package           | Version            | License      | Dependency Relationship |
|-------------------|--------------------|--------------|-------------------------|
| networkx          | 3.2                | BSD          |            —            |
| numpy             | 1.26               | BSD-3-Clause |            —            |
| pandas            | 2.1                | BSD-3-Clause |            —            |
| scikit-learn      | 1.5.0              | BSD-3-Clause |            —            |
| joblib            | 1.3                | BSD License  |            —            |
| graphviz          | 0.20               | EPL-1.0      |            —            |
| fastapi           | 0.110              | MIT          |            —            |
| uvicorn           | 0.27               | BSD-3-Clause |            —            |
| anyio             | 4.2                | MIT          |            —            |
| sniffio           | 1.3                | MIT          |            —            |
| pydantic          | 2.6                | MIT          |            —            |
| pyyaml            | 6.0                | MIT          |            —            |
| orjson            | 3.9                | Apache-2.0   |            —            |
| httpx             | 0.26               | BSD-3-Clause |            —            |
| requests          | 2.32.4             | Apache-2.0   |            —            |
| rich              | 13.7               | MIT          |            —            |
| tqdm              | 4.66.3             | MPL-2.0      |            —            |
| matplotlib        | 3.8                | PSF License  |            —            |
| seaborn           | 0.12               | BSD-3-Clause |            —            |
| pillow            | 12.1.1             | HPND License |            —            |
| pytest            | 8.0                | MIT          |            —            |
| pytest-asyncio    | 0.23               | Apache-2.0   |            —            |
| pytest-cov        | 4.1                | MIT          |            —            |
| pylint            | 3.0                | GPL-2.0      |            —            |
| flake8            | 7.0                | MIT          |            —            |
| black             | 24.0               | MIT          |            —            |
| isort             | 5.13               | MIT          |            —            |
| autoflake         | 2.3                | MIT          |            —            |
| build             | 1.2                | MIT          |            —            |
| loguru            | 0.7                | MIT          |            —            |
