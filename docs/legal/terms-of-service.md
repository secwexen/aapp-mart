# Terms of Service

## 1. Acceptance of Terms  

By accessing or using AAPP-MART, you agree to be bound by these Terms of Service. If you do not agree, you must discontinue use of the platform.

## 2. Description of Service  

AAPP-MART provides digital tools and features intended to support users in their online activities. The platform may evolve over time through updates, improvements, or new functionality.

## 3. User Responsibilities  

Users agree to:   
- Maintain the confidentiality of their account credentials and are responsible for all activities conducted under their account.
- Ensure all information provided to the platform is accurate and lawful.  
- Use the service only for legal and authorized purposes.  
- Avoid actions that may disrupt or harm the platform or other users.

## 4. Prohibited Activities  

The following activities are not allowed:  
- Unauthorized access attempts or security breaches.  
- Uploading harmful, malicious, or illegal content.  
- Interfering with platform functionality or performance.  
- Violating intellectual property rights.  
- Attempting to access other users’ data without permission.

## 5. Open Source License

AAPP-MART is distributed under the Apache License 2.0. Users may use, modify, reproduce, and distribute the software in accordance with the terms of the license. The full license text is available in the [LICENSE](/LICENSE) file included with the project.

## 6. Intellectual Property  

The AAPP-MART source code is provided under the Apache License 2.0. Users may use, modify, reproduce, and distribute the software in accordance with the terms of the license, and all copyright notices and license headers must be preserved in any copies or derivative works. The AAPP-MART name, logo, and branding elements are not covered by the open-source license and may not be used without explicit permission.

## 7. Data Usage  

User data is processed solely for the purpose of providing and improving the service. Details about data collection and processing are described in the [Privacy Policy](docs/legal/privacy-policy.md) document.

## 8. Service Availability  

AAPP-MART aims to provide continuous access to the platform but does not guarantee uninterrupted service. Maintenance, updates, or technical issues may temporarily affect availability.

## 9. Limitation of Liability  

AAPP-MART is provided “as is” without warranties of any kind. The platform is not liable for any direct or indirect damages resulting from use of the service.

## 10. Changes to Terms  

AAPP-MART may update these Terms of Service when necessary. Continued use of the platform after updates constitutes acceptance of the revised terms.
